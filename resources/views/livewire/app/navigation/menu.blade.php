        ...
        <div class="grid grid-cols-4 py-2">
            @foreach ($this->menuItems() as $item)
                <a
                    ...
                    wire:navigate
                    class="flex flex-col items-center gap-1 px-2 py-1 {{ request()->routeIs(data_get($item, 'route')) ? 'text-primary' : 'text-primary-400' }}"
                >
                    <x-dynamic-component
                        :component="data_get($item, 'icon')"
                        @class([
                            'h-5 w-5 shrink-0',
                            'text-primary-800' => request()->routeIs(data_get($item, 'route')),
                        ])
                    />
                    <span
                            @class([
